...
async function action(name, data = {}) {
  ...
  message(name === 'dark' && !data.clear ? 'Acquiring dark reference. Keep the beam blocked until this message disappears…' :
    name === 'auto_exposure' ? 'Adjusting exposure…' : '');
  renderState();
  try {
    state = await api(name, data);
    syncFields();
    message(state.error || (name === 'auto_exposure' ? state.auto_exposure?.message : '') || '');
...
function renderState() {
  ...
  for (const id of ['pause', 'apply-camera', 'auto-exposure', 'exposure', 'gain', 'dark', 'roi-tool', 'full-roi']) $(id).disabled = !connected || busy;
  // A replay keeps its recorded exposure, gain and dark references.
  for (const id of ['apply-camera', 'auto-exposure', 'exposure', 'gain', 'dark']) if (replay) $(id).disabled = true;
...
$('auto-exposure').onclick=()=>action('auto_exposure');
